...
def main():
    main_page = get_page_content(COMPETITION_URL, "feminino_sub19_competition_main_105")
    if not main_page:
    ...
    for round_data in rounds:
        present = {entry["team"] for entry in round_data["classification"]}
        missing = [team for team in all_teams if team not in present]
        if missing:
            for team in sorted(missing):
                round_data["classification"].append({
                    "position": 0,
                    "team": team,
                    "played": 0,
                    "wins": 0,
            ...
            for idx, entry in enumerate(round_data["classification"], start=1):
                entry["position"] = idx
