...
def main():
    main_page = get_page_content(COMPETITION_URL, "feminino_sub17_competition_main")
    if not main_page:
    ...
    for index, fixture_id in enumerate(fixture_ids, start=1):
        ...
        fragment = get_page_content(url, f"feminino_sub17_fixture_{fixture_id}")
        if not fragment:
