...
def main():
    main_page = get_page_content(COMPETITION_URL, "juniores_competition_main")
    if not main_page:
    ...
    for index, fixture_id in enumerate(fixture_ids, start=1):
        ...
        fragment = get_page_content(url, f"juniores_fixture_{fixture_id}")
        if not fragment:
