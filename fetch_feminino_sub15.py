...
COMPETITION_URL = "https://resultados.fpf.pt/Competition/Details?competitionId=28962&seasonId=105"
OUTPUT_FILE = "data/feminino-sub15.json"
CACHE_DIR = "cache"
USE_CACHE = False
# Competição só tem uma série, por isso deixamos o alvo vazio
TARGET_SERIE_NAME = ""
...
def extract_fixture_ids(html_content: str, target_serie: str):
    normalized_target = _normalize(target_serie) if target_serie else ''
    pattern = re.compile(r'<div class="game-results[^>]*id="htmlSerieId_(\d+)"[^>]*>')
    last_fallback_block = None
    ...
    for match in re.finditer(pattern, html_content):
        ...
        if not last_fallback_block:
            last_fallback_block = block
    ...
    if last_fallback_block:
        fixture_ids = []
        for fixture_id in re.findall(r'fixtureId=(\d+)', last_fallback_block):
            if fixture_id not in fixture_ids:
                fixture_ids.append(fixture_id)
        if fixture_ids:
            return fixture_ids
...
def main():
    main_page = get_page_content(COMPETITION_URL, "feminino_sub15_competition_main")
    if not main_page:
    ...
    for index, fixture_id in enumerate(fixture_ids, start=1):
        ...
        fragment = get_page_content(url, f"feminino_sub15_fixture_{fixture_id}")
        if not fragment:
